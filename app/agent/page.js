...
export default function Home() {
  ...
  const handleManualSync = async () => {
    if (!confirm('전 세계 모든 플랫폼의 데이터를 최신화하시겠습니까? 약 5~10분이 소요될 수 있습니다.')) return;
    setIsSyncing(true);
    try {
      const res = await fetch('/api/ducktem/sync', { method: 'POST' });
      if (res.ok) alert('🚀 글로벌 동기화 엔진이 가동되었습니다! GitHub Actions 탭에서 진행 상황을 확인하세요.');
      else alert('동기화 요청 실패');
    } catch (err) {
      alert('오류가 발생했습니다.');
    }
    setIsSyncing(false);
        ...
        <header className="mb-10 flex justify-between items-end">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              disabled={isSyncing}
              className="px-6 py-3 bg-green-600 text-white rounded-xl font-black hover:bg-green-700 transition-all shadow-md flex items-center gap-2"
            >
              {isSyncing ? '동기화 중...' : '🔄 전 세계 데이터 동기화'}
            </button>
            <Link href="/" className="px-6 py-3 bg-yellow-400 text-yellow-900 rounded-xl font-black hover:bg-yellow-500 transition-all shadow-md flex items-center gap-2">
              🦆 DUCKTEM 홈으로
            </Link>
            {user ? (
